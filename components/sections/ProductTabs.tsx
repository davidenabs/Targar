                ...
                return (
                  <li key={tab.id} className="relative">
                    <button
                      ...
                      className={`text-[10px] font-bold uppercase tracking-[0.2em] transition-all duration-300 flex items-center gap-4 ${isActive ? "text-brand-pink" : "text-gray-400 hover:text-gray-600"
                        }`}
                    >
                      {isActive && (
                        <motion.span
            ...
            {tabs.map((tab, idx) => {
              ...
              return (
                <button
                  ...
                  className={`text-[10px] whitespace-nowrap font-bold uppercase tracking-[0.2em] transition-all duration-300 ${isActive ? "text-brand-pink border-b-2 border-brand-pink pb-1" : "text-gray-400"
                    }`}
                >
                  {tab.label}
                </button>
          ...
          <div className={`flex-1 w-full ${isStickyScroll ? 'min-h-[600px] lg:min-h-[640px]' : ''} relative`}>
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {/* Cards Grid */}
                <div className="w-full xl:w-[100%] grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
                  {activeTab.features.map((feature: any, idx: number) => (
                    <div key={idx} className="bg-white rounded-[1.25rem] p-6 lg:p-8 shadow-[0_4px_24px_rgba(0,0,0,0.03)] border border-gray-50 transition-all duration-300 hover:-translate-y-1">
                      <div className="w-10 h-10 rounded-full bg-indigo-50/80 flex items-center justify-center text-brand-pink mb-6">
                      ...
                      <a href="#" className="text-[11px] font-bold text-brand-pink flex items-center gap-1 hover:gap-2 transition-all">
